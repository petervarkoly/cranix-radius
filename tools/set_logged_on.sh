#!/bin/bash
# This script sends the MAC address and the user name to the cranix-api.
...
# If the registration was not successfully the client stands in the ANON_DCHP range or braek the connection if CRANIX_FORCE_REGISTER_DEVICE set yes
# Copyright Dipl.-Ing. Peter Varkoly <pvarkoly@cephalix.eu>
. /etc/sysconfig/cranix
if [ "$2" ]
then
	MAC=$( echo $2 |  tr "-" ":" )
	if [ "${CRANIX_RADIUS_REGISTER_DEVICE}" = "yes" ]
	then
		...
		case "$OUT" in
			...
			OK*)
				...
				;;
			*)
				if [ "${CRANIX_FORCE_REGISTER_DEVICE}" = "yes" ]; then
